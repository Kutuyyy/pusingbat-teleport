--[[
    Pusingbat Hub (Server-backed via ngrok)
    ...
    • Search memfilter konten tab aktif
    • Close -> sembunyikan panel, munculkan pill "Show Pusing"
    • Minimize -> header saja (klik lagi untuk restore)
    • ScrollingFrame per tab (panel tidak terlalu tinggi; nyaman di HP)
    ...
    TELEPORT:
      - Add Current Location (minta nama)
      - Delete Selected
      - Export Locations (minta nama) -> SIMPAN KE SERVER
      - Import Locations (pilih dari daftar di SERVER)
      - Klik entry untuk teleport

    CONFIG (SEMUA VIA SERVER):
      - Input nama config
      - Save Config (ke server)
      - Daftar Config (Load / Auto / Delete)
      - Auto Load: diterapkan otomatis saat UI muncul
      - Status server (ONLINE/OFFLINE) tampil di header

    Server API (ngrok):
      GET  /v1/users/<hwid> -> {autoload, configs, exports}
      PUT  /v1/users/<hwid> -> body full (autoload/configs/exports/meta.username)
      POST /v1/usage        -> {username, hwid}
      GET  /health          -> tanpa API key (opsional untuk tes)
]]--

-- ==== SERVER CONFIG (NGROK) ====
local API_URL = "https://9af2851a399d.ngrok-free.app"  -- GANTI jika URL ngrok berubah
local API_KEY = "asdasdasdasdasdasdasdasd"
local DEBUG_HTTP = true  -- true: tampilkan log [PB-HTTP] di F9 Console
...
local function dprint(...)
	if DEBUG_HTTP then print("[PB-HTTP]", ...) end
end

-- executor-friendly HTTP: syn.request / http_request / request / HttpService:RequestAsync
local function lowLevelRequest(method, url, headers, body)
	local req = (syn and syn.request) or (http and http.request) or http_request or request
	if req then
		local ok, res = pcall(req, { Url=url, Method=method, Headers=headers, Body=body })
		if not ok then
			return false, { Success=false, StatusCode="exec-error", Body=tostring(res) }
		end
		local success = (res.StatusCode and res.StatusCode >= 200 and res.StatusCode < 300)
		return true, { Success=success, StatusCode=res.StatusCode or 0, Body=res.Body or "" }
	end
	local ok, res = pcall(function()
		return HttpService:RequestAsync({ Url=url, Method=method, Headers=headers, Body=body })
	end)
	if not ok then
		return false, { Success=false, StatusCode="rbx-error", Body=tostring(res) }
	end
	return true, res
end
...
local function request(method, path, bodyTbl)
	...
	dprint("REQ", method, url)
	local ok, res = lowLevelRequest(method, url, headers, body)
	if not ok or not res then
		dprint("FAIL(nores)", method, url)
		return false, "request-failed"
	end
	if not res.Success then
		dprint("HTTP-FAIL", method, url, "code:", res.StatusCode, "body:", tostring(res.Body))
		return false, res.StatusCode or "http-error"
	end
	local data
	if res.Body and res.Body ~= "" then
		local good, parsed = pcall(function() return HttpService:JSONDecode(res.Body) end)
		if good then data = parsed else dprint("JSON-parse-fail", tostring(res.Body)) end
	end
	dprint("OK", method, url)
	return true, data
end

-- API helpers
local function apiGetUser(hwid)          return request("GET",  "/v1/users/"..hwid) end
local function apiPutUser(hwid, userObj) return request("PUT",  "/v1/users/"..hwid, userObj) end
local function apiUsage(username, hwid)  return request("POST", "/v1/usage", { username = username, hwid = hwid }) end
...
local savedLocations = {}           -- { {name, position=Vector3, selected, checkbox} }
local exportedSets = {}             -- name -> array of {name, position}
local configs = {}                  -- name -> settings table
local autoloadName = nil
local serverOnline = false          -- diupdate saat fetch awal
local serverStatusLabel -- UI label kecil di header
...
local function setFly(state)
	...
	if fly then
		...
		if lv then lv.VectorVelocity = Vector3.zero; lv.Enabled = false end
		if align then align:Destroy(); align = nil end
...
RunService.RenderStepped:Connect(function()
	...
	if dir.Magnitude > 0 then velocity = velocity + dir.Unit * walkSpeed end
...
local function createUI()
	...
	textBg.BackgroundColor3 = Color3.new(0,0,0)
	...
	-- Panel utama (mulai disabled, baru enable setelah 5s)
	...
	frame.Size = UDim2.fromOffset(440, 420) -- lebih pendek biar nyaman di HP
	...
	-- Drag area (supaya tombol bisa diklik)
	...
	local function makeTabButton(text, x)
		...
		b.Size = UDim2.fromOffset(102, 28)
	...
	local tabMiscBtn = makeTabButton("Misc", 106)
	local tabTpBtn   = makeTabButton("Teleport", 212)
	local tabCfgBtn  = makeTabButton("Config", 318)

	-- Scrolling containers per tab
	local function makeScroll()
		...
		lay:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			s.CanvasSize = UDim2.new(0,0,0, lay.AbsoluteContentSize.Y + 20)
		end)
		return s
	end

	local mainScroll = makeScroll()
	local miscScroll = makeScroll()
	...
	local function makeSwitch(parent, labelText, initial, callback)
		...
		row:SetAttribute("label", string.lower(labelText))
	...
	exportBtn.Text = "Export Locations (Server)"
	...
	local function createLocationEntry(locationData)
		...
		entry:SetAttribute("label", string.lower(locationData.name or ""))
	...
	exportBtn.MouseButton1Click:Connect(function()
		...
		promptName(defaultName, "Export as:", function(name)
			...
			-- push ke server
			local body = { autoload = autoloadName, configs = configs, exports = exportedSets, meta = {username=USERNAME} }
			local ok,_ = apiPutUser(HWID, body)
			if not ok then dprint("export PUT failed") end
		end)
	end)

	importBtn.MouseButton1Click:Connect(function()
		-- tampilkan list exportedSets (server state terakhir)
		...
		for name,set in pairs(exportedSets) do
			...
			b.MouseButton1Click:Connect(function()
				...
				for _,loc in ipairs(set) do
					local nd = {name=loc.name, position=Vector3.new(loc.position.X,loc.position.Y,loc.position.Z), selected=false}
	...
	local function getCurrentSettings()
		return {
			...
			infJumpMobile=infJumpMobile, infJumpPC=infJumpPC,
		...
		for name, s in pairs(configs) do
			...
			autoB.MouseButton1Click:Connect(function()
				...
				if serverOnline then
					local body = { autoload = autoloadName, configs = configs, exports = exportedSets, meta = {username=USERNAME} }
					local ok,_ = apiPutUser(HWID, body)
					if not ok then dprint("set auto PUT failed") end
				end
				rebuildCfgList()
			end)
			delB.MouseButton1Click:Connect(function()
				...
				if serverOnline then
					local body = { autoload = autoloadName, configs = configs, exports = exportedSets, meta = {username=USERNAME} }
					local ok,_ = apiPutUser(HWID, body)
					if not ok then dprint("delete PUT failed") end
				end
				rebuildCfgList()
			end)
		end
	...
	saveBtn.MouseButton1Click:Connect(function()
		...
		if not serverOnline then dprint("save: server offline"); rebuildCfgList(); return end
		local body = { autoload = autoloadName, configs = configs, exports = exportedSets, meta = {username=USERNAME} }
		local ok,_ = apiPutUser(HWID, body)
		if not ok then dprint("save PUT failed") end
		rebuildCfgList()
	end)
	...
	task.delay(5, function()
		...
		-- apply autoload jika ada & server berhasil load
		...
		if serverStatusLabel then
			serverStatusLabel.Text = serverOnline and "[ONLINE]" or "[OFFLINE]"
			serverStatusLabel.TextColor3 = serverOnline and Color3.fromRGB(140,220,140) or Color3.fromRGB(220,140,140)
		end
		-- Refresh list config (UI)
		rebuildCfgList()
	end)
end
...
-- Ping usage (tidak wajib, untuk penggunaa.json di server)
task.spawn(function()
	local ok,_ = apiUsage(USERNAME, HWID)
	if not ok then dprint("usage ping failed") end
end)

-- Fetch data dari server (autoload/configs/exports)
task.spawn(function()
	local ok, data = apiGetUser(HWID)
	serverOnline = ok and (type(data)=="table")
	if serverOnline and data then
		autoloadName = data.autoload
		configs = data.configs or {}
		exportedSets = data.exports or {}
	else
		autoloadName, configs, exportedSets = nil, {}, {}
	end
	-- Update status label kalau UI sudah ada
	if serverStatusLabel then
		serverStatusLabel.Text = serverOnline and "[ONLINE]" or "[OFFLINE]"
		serverStatusLabel.TextColor3 = serverOnline and Color3.fromRGB(140,220,140) or Color3.fromRGB(220,140,140)
	end
