--[[
    ...
    Fitur:
      ...
      • ScrollingFrame per tab (mobile-friendly, panel tinggi adaptif)
      ...
      • TELEPORT:
          ...
          - Export Locations (minta nama) → tersimpan ke file .json
          - Import Locations (list exports dari file)
...
local function createUI()
	...
	-- Panel size adaptif (mobile-friendly)
	local vp = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(1280,720)
	local targetH = math.clamp(vp.Y * 0.6, 340, 520) -- 60% tinggi layar, min 340, max 520

	local frame = Instance.new("Frame")
	frame.Name = "MainFrame"
	frame.Size = UDim2.fromOffset(480, targetH)
	frame.Position = UDim2.new(0, 24, 0, 100) -- agak turun dari top supaya tidak mepet
	...
	exportBtn.MouseButton1Click:Connect(function()
		...
		promptName(defaultName, "Export as:", function(name)
			...
			saveExport(name, copy) -- persist ke file kalau FS tersedia
		end)
	end)
	...
	local function applySearch()
		...
		elseif activeTab == "Teleport" then
			-- keep top button bars always visible
			local q = string.lower(searchBox.Text or "")
	...
	btnMin.MouseButton1Click:Connect(function()
		...
		frame.Size = minimized and UDim2.fromOffset(480, 56) or UDim2.fromOffset(480, targetH)
	end)
	...
	task.delay(5, function()
		...
		if autoloadName and configs[autoloadName] then
			...
			task.defer(function() applySettings(s) end)
		end
		-- Set FOV awal
		setFOV(defaultFOV)
	end)
...
-- ====== Init ======
loadAllFromDisk() -- load local settings/config/export duluan
